...
Eight agents run the sequential delivery pipeline: read a Multica request or ADO deliverable -> determine planning source -> guided planning when needed -> create/reuse tasks -> plan -> implement -> test -> review -> refine -> PR. A ninth Watchdog agent runs stall recovery. The master Multica issue carries all pipeline state; no local state files.
...
- Guided planning from the ADO deliverable or direct Multica issue content, using a Grill-with-docs one-question-at-a-time review before tasks are proposed.
...
```bash
multica skill import --url https://github.com/shadcn/improve/blob/main/skills/improve/SKILL.md --output json
```

Also import the upstream Grill-with-docs stack for Coding Team Guided Planner:

```bash
multica skill import --url https://github.com/mattpocock/skills/blob/main/skills/engineering/grill-with-docs/SKILL.md --output json
multica skill import --url https://github.com/mattpocock/skills/blob/main/skills/engineering/domain-modeling/SKILL.md --output json
multica skill import --url https://github.com/mattpocock/skills/blob/main/skills/productivity/grilling/SKILL.md --output json
```
...
| `dettools/coding_handoff_decide.go` | `coding_handoff_decide` | Implementer, Test Writer, Reviewer, Watchdog |
...
- **Skills:** `shared-ado-ops`, `shared-state-ops`, `guided-planner`, `grill-with-docs`, `domain-modeling`, `grilling`
- **Instructions:** You run Grill-with-docs guided planning for coding-team master issues. Ask one question at a time, record resolved terms and decision notes, and produce proposed implementation tasks for approval.
- **Max concurrent tasks:** 1
...
- **Instructions:** You conduct strict code reviews against acceptance criteria and coding standards. On PASS, hand off to Coding Team Refiner; on FAIL, route back to Coding Team Implementer.
- **Max concurrent tasks:** 1

### Coding Team Refiner
- **Name:** `Coding Team Refiner`
- **Skills:** `shared-ado-ops`, `shared-state-ops`, `refiner`, `improve`
- **Instructions:** You run the separate post-review refinement pass. Use /improve-style branch analysis in read-only mode, route blocking refinements back to Implementer, or notify Orchestrator with TASK_COMPLETE.
- **Max concurrent tasks:** 1
...
Set both on all 8 pipeline agents via the Multica agent `custom_env` field. The skills use inline `AZURE_DEVOPS_EXT_PAT=` prefixes to route the right PAT to the right ADO instance — never let one bleed into the other. Watchdog does not need ADO PATs.
...
1. Create a new Multica issue with this description:
   ...
   | `guided` | Coding Team Guided Planner should question the plan one decision at a time, then Orchestrator creates tasks after approval. |
   ...
   Guided planning follows the Grill-with-docs interaction pattern: Coding Team Guided Planner asks one question at a time, includes its recommended answer, records resolved terms and decision notes in master issue state, and proposes tasks when ready. For Multica-only runs, the first Orchestrator run must route to Guided Planner and stop before task synthesis or Planner handoff. Guided Planner may use ADO context when available, but it must not check out or inspect the repo during guided planning; Planner owns codebase exploration after approval.

2. Assign the issue directly to **Coding Team Orchestrator**. Do not assign coding pipeline issues to a squad.

3. The Orchestrator reads the request, fetches ADO only when `deliverable_id` is present, and determines the planning source. For guided planning, it routes to Coding Team Guided Planner, which asks one decision question at a time. Reply with your answer, or reply **use recommendation** to accept the recommended answer.

4. After guided planning is complete, or immediately for existing/decomposition paths, the task list is proposed. Reply **approve** (or provide feedback for revision). The Orchestrator reuses/creates ADO tasks only for ADO-backed runs, creates Multica child issues and the feature branch, then kicks off the Planner on the first task.

5. The pipeline runs automatically through plan → implement → test → review → refine for each task sequentially. During planning, the Planner performs the first repository checkout and uses the deliverable, task details, optional ADO Component, and codebase structure to choose the owning code project/module.
...
|-------|-----------------|
| `guided_planning` | Coding Team Guided Planner asking one guided planning question at a time before task proposal |
