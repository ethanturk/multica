...
description: Multica drives whichever AI coding tools you have on your machine. This page shows you how to install each of the 17 supported tools so the daemon can detect them.
---

import { Callout } from "fumadocs-ui/components/callout";

A **runtime** in Multica is the daemon on your machine paired with one AI coding tool the daemon found on your `PATH`. If the onboarding "Connect a runtime" step shows **No supported tools detected**, it means the daemon scanned `PATH` and didn't find any of the 17 tools it knows how to drive. Install one (or several) of the tools below, then come back to the step and re-scan — the runtime will show up within a few seconds.
...
## The 17 supported tools

Listed roughly from most to least common. Pick whichever ones you already have credentials for — you don't need all 17.
...
For authenticated MCP servers, Cursor stores OAuth state in the project data directory where the auth flow first ran. Multica isolates managed Cursor MCP runs with a per-task `CURSOR_DATA_DIR`, so it does not scan or copy every file under `~/.cursor/projects`. To opt in for one agent, set the agent custom env key `CURSOR_MCP_AUTH_SOURCE` to the absolute path of the trusted `mcp-auth.json` file, or to the Cursor project data directory containing that file. Multica links or seeds only that `mcp-auth.json` into the isolated task project data directory. When symlinks are supported, Cursor token refreshes may write back to the trusted source; environments that cannot create symlinks fall back to copying the file.

### Dirge

Headless Dirge tasks use `dirge --print --output-format json`. Session resumption works through `--session <id>`, MCP config is written to a per-task `DIRGE_CONFIG_DIR/config.json`, and skills live under `.dirge/skills/`.

| | |
|---|---|
| Daemon looks for | `dirge` |
| Install | Follow the official guide at [dirge-code.github.io](https://dirge-code.github.io/). |
| Authentication | Configure Dirge through its own login or provider setup flow before starting the Multica daemon. |
| Notes | Set `MULTICA_DIRGE_PATH` to pin a non-standard binary path, and `MULTICA_DIRGE_MODEL` to seed the default model passed via `--model`. |
...
**DevEco Code** is Huawei's independent coding agent for HarmonyOS, built on the OpenCode engine. It ships its own model catalog and Huawei-account auth. Multica drives it over `deveco run --format json`; models are discovered dynamically from `deveco models` (built-in `deveco/GLM-5.1`). MCP servers are configured through DevEco's native config (`DEVECO_CONFIG_CONTENT`); Multica-side `mcp_config` wiring is in progress.

| | |
|---|---|
| Daemon looks for | `deveco` |
| Install | `npm install -g @deveco/deveco-code`. See the official README at [gitcode.com/openharmony-sig/deveco-code](https://gitcode.com/openharmony-sig/deveco-code/blob/develop/README.en.md) or the npm page at [npmjs.com/package/@deveco/deveco-code](https://www.npmjs.com/package/@deveco/deveco-code). |
...
Qwen Code uses its native JSONL protocol in daemon mode: `qwen -p <prompt> --output-format stream-json --yolo`. Multica owns `--yolo` for unattended runs so shell, edit, and write tools remain available without an approval UI. This does not enable Qwen's sandbox: unless the deployment explicitly enables `--sandbox` or `QWEN_SANDBOX`, tools run with the daemon OS user's permissions and credential scope. Use `--exclude-tools` in custom arguments to hard-deny specific tools. Session resumption works through `--resume`; the task runtime brief is written to `QWEN.md`; assigned skills are copied to `.qwen/skills/`; and personal skills use `$QWEN_HOME/skills/` (default `~/.qwen/skills/`).

| | |
|---|---|
| Daemon looks for | `qwen` |
| Install | Install Qwen Code using Alibaba's current Qwen Code instructions, then ensure `qwen` is on `PATH`. |
...
| Notes | Override the binary with `MULTICA_GROK_PATH` and the default model with `MULTICA_GROK_MODEL` (e.g. `grok-4.5`). Minimum CLI version: `0.2.89`. |

## After installing

1. **Confirm the binary is on `PATH`.** Open a fresh terminal and run `which <name>` (for example `which claude`, `which cursor-agent`, `which kiro-cli`, `which agy`, `which dirge`). If it prints a path, the daemon will find it. If it prints nothing, fix your shell `PATH` first (the typical cause is a per-shell rc file that wasn't reloaded).
