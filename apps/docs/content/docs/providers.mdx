...
| **Dirge** | Dirge | ✅ (`--session <id>`) | ✅ | `.dirge/skills/` | Custom flag |
...
From the Dirge project. Multica runs `dirge --print --output-format json` for headless tasks, passes the selected model with `--model`, and pins resumable context with `--session <id>`. MCP config is materialized into a per-task `DIRGE_CONFIG_DIR/config.json` as `mcp_servers`, preserving the user's other Dirge config keys while replacing only the managed MCP server block. Skills are copied into `.dirge/skills/`.
...
From xAI. CLI binary name is `grok` (Grok Build CLI, often installed to `~/.grok/bin/grok`). Multica drives it over **ACP** via `grok --no-auto-update agent --always-approve stdio`, reusing the same transport as Trae / Kimi / Kiro / Qoder. After `initialize` Multica completes xAI's headless **auth handshake** (`authenticate`, preferring `xai.api_key` when `XAI_API_KEY` is set, else the cached login token) before any session operation. **Session resumption works** through ACP `session/load`. **Model selection works** via the catalog returned from `session/new` and `session/set_model`. **Reasoning effort** is exposed through Multica’s thinking picker and passed as `--effort` on the `agent` command. **MCP is supported** — config is passed through ACP `mcpServers` (filtered to transports advertised by `initialize`). Skills land in `.grok/skills/` (project) and `~/.grok/skills` (user); the CLI also scans `.agents/skills/`. Tool calls stream as ACP `tool_call` / `tool_call_update` events.
...
**Of the 15 tools, twelve consume `mcp_config`: Claude Code, CodeBuddy, Codex, Cursor, Dirge, Hermes, Kimi, Kiro CLI, OpenCode, OpenClaw, Qoder, and Trae**. The other three (Antigravity, Copilot, Pi) accept the field but **ignore it** — no error, no warning, the config just has no effect., eleven consume `mcp_config`: Claude Code, and Trae**. The other four (Antigravity, DevEco Code

The runtime paths are provider-specific: Claude Code and CodeBuddy receive it through `--mcp-config` paired with `--strict-mcp-config`; Codex writes a daemon-managed `mcp_servers` block into the per-task `$CODEX_HOME/config.toml`; Cursor writes `.cursor/mcp.json` plus per-task project approvals under `CURSOR_DATA_DIR`; Dirge writes a daemon-managed `mcp_servers` block into a per-task `DIRGE_CONFIG_DIR/config.json`; Hermes, Kimi, Kiro CLI, Qoder, and Trae receive ACP `mcpServers`; OpenCode receives inline config through `OPENCODE_CONFIG_CONTENT`; OpenClaw receives `mcp.servers` through Multica's per-task config wrapper. OpenCode's path does **not** rewrite the project's `opencode.json`.
