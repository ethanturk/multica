...
func TestBuildCommentReplyInstructionsNonCodexLinux(t *testing.T) {
	...
	for _, host := range []string{"linux", "darwin"} {
		for _, provider := range []string{"claude", "opencode", "openclaw", "hermes", "kimi", "reasonix", "kiro", "cursor", "dirge"} {
			name := provider + "/" + host
...
func TestBuildCommentReplyInstructionsWindowsUsesContentFile(t *testing.T) {
	...
	for _, provider := range []string{"codex", "claude", "opencode", "openclaw", "hermes", "kimi", "reasonix", "kiro", "cursor", "dirge"} {
